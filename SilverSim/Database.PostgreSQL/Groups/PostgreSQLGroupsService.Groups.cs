...
namespace SilverSim.Database.PostgreSQL.Groups
{
    public sealed partial class PostgreSQLGroupsService : GroupsServiceInterface.IGroupsInterface
    {
        ...
        bool IGroupsInterface.ContainsKey(UUI requestingAgent, string groupName)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT \"GroupID\" FROM groups WHERE \"Name\" = @groupname", conn))
                {
        ...
        bool IGroupsInterface.ContainsKey(UUI requestingAgent, UUID groupID)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT \"GroupID\" FROM groups WHERE \"GroupID\" = @groupid", conn))
                {
        ...
        bool IGroupsInterface.TryGetValue(UUI requestingAgent, string groupName, out GroupInfo groupInfo)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT g.*, " + GCountQuery + " FROM groups AS g WHERE g.\"Name\" = @groupname", conn))
                {
                    cmd.Parameters.AddParameter("@groupname", groupName);
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
                    {
        ...
        bool IGroupsInterface.TryGetValue(UUI requestingAgent, UGI group, out GroupInfo groupInfo)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT g.*, " + GCountQuery + " FROM groups AS g WHERE g.\"GroupID\" = @groupid", conn))
                {
                    cmd.Parameters.AddParameter("@groupid", group.ID);
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
                    {
        ...
        bool IGroupsInterface.TryGetValue(UUI requestingAgent, UUID groupID, out UGI ugi)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT \"Name\", \"Location\" FROM groups WHERE \"GroupID\" = @groupid", conn))
                {
                    cmd.Parameters.AddParameter("@groupid", groupID);
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
                    {
