    ...
    public sealed partial class PostgreSQLGroupsService : IGroupMembersInterface
    {
        List<GroupMember> IGroupMembersInterface.this[UGUI requestingAgent, UGUI principal]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT m.* FROM groupmemberships AS m WHERE m.\"PrincipalID\" = @principalid", conn))
                    {
        ...
        bool IGroupMembersInterface.ContainsKey(UGUI requestingAgent, UGI group, UGUI principal)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT \"GroupID\" FROM groupmemberships WHERE \"GroupID\" = @groupid AND \"PrincipalID\" = @principalid LIMIT 1", conn))
                {
                    cmd.Parameters.AddParameter("@groupid", group.ID);
                    cmd.Parameters.AddParameter("@principalid", principal.ID);
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
        ...
        void IGroupMembersInterface.Delete(UGUI requestingAgent, UGI group, UGUI principal)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("DELETE FROM groupmemberships WHERE \"GroupID\" = @groupid AND \"PrincipalID\" = @principalid", conn))
                {
                    cmd.Parameters.AddParameter("@principalid", principal.ID);
                    cmd.Parameters.AddParameter("@groupid", group.ID);
        ...
        bool IGroupMembersInterface.TryGetValue(UGUI requestingAgent, UGI group, UGUI principal, out GroupMember gmem)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT * FROM groupmemberships WHERE \"GroupID\" = @groupid AND \"PrincipalID\" = @principalid LIMIT 1", conn))
                {
                    cmd.Parameters.AddParameter("@groupid", group.ID);
                    cmd.Parameters.AddParameter("@principalid", principal.ID);
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
        ...
        void IGroupMembersInterface.Update(UGUI requestingagent, UGI group, UGUI principal, bool acceptNotices, bool listInProfile)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("UPDATE groupmemberships SET \"AcceptNotices\"=@acceptnotices, \"ListInProfile\"=@listinprofile WHERE \"GroupID\" = @groupid AND \"PrincipalID\" = @principalid", conn))
                {
