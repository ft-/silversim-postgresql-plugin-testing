...
{
    public sealed partial class PostgreSQLGroupsService : IAssetReferenceInfoServiceInterface
    {
        public void EnumerateUsedAssets(Action<UUID> action)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT DISTINCT \"InsigniaID\" FROM groups", conn))
                {
