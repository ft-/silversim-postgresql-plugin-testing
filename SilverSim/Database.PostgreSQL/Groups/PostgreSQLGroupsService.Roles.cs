    ...
    public sealed partial class PostgreSQLGroupsService : GroupsServiceInterface.IGroupRolesInterface
    {
        List<GroupRole> IGroupRolesInterface.this[UUI requestingAgent, UGI group]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT r.*," + RCountQuery + " FROM grouproles AS r WHERE r.\"GroupID\" = @groupid", conn))
                    {
        ...
        List<GroupRole> IGroupRolesInterface.this[UUI requestingAgent, UGI group, UUI principal]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT r.*," + RCountQuery + " FROM grouprolememberships AS rm INNER JOIN grouproles AS r ON rm.\"GroupID\" AND r.\"GroupID\" AND rm.\"RoleID\" = r.\"RoleID\" WHERE r.\"GroupID\" = @groupid AND rm.\"PrincipalID\" = @principalid", conn))
                    {
                        cmd.Parameters.AddParameter("@groupid", group.ID);
        ...
        void IGroupRolesInterface.Delete(UUI requestingAgent, UGI group, UUID roleID)
        {
            ...
            using (var conn = new NpgsqlConnection(m_ConnectionString))
            {
                ...
                {
                    using (var cmd = new NpgsqlCommand("UPDATE groupmemberships SET \"SelectedRoleID\"=@zeroid WHERE \"SelectedRoleID\" = @roleid", conn)
                    {
                    ...
                    {
                        using (var cmd = new NpgsqlCommand("DELETE FROM " + table + " WHERE \"GroupID\" = @groupid AND \"RoleID\" = @roleid", conn)
                        {
                            Transaction = transaction
                        })
                        {
        ...
        void IGroupRolesInterface.Update(UUI requestingAgent, GroupRole role)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("UPDATE grouproles SET \"Name\"=@name, \"Description\"=@description, \"Title\"=@title,\"Powers\"=@powers WHERE \"GroupID\" = @groupid AND \"RoleID\" = @roleid", conn))
                {
