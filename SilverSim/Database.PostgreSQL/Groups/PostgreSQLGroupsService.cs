...
{
    [Description("PostgreSQL Groups Backend")]
    [PluginName("Groups")]
    public sealed partial class PostgreSQLGroupsService : GroupsServiceInterface, IPlugin, IDBServiceInterface, IUserAccountDeleteServiceInterface
    {
        ...
        private bool TryGetGroupRoleRights(UUI requestingAgent, UGI group, UUID roleID, out GroupPowers powers)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT Powers FROM grouproles AS r WHERE r.\"GroupID\" = @groupid AND r.\"RoleID\" = @grouproleid", conn))
                {
