...
{
    public sealed partial class PostgreSQLGroupsService : GroupsServiceInterface.IGroupRolemembersInterface
    {
        ...
        List<GroupRolemembership> IGroupRolemembersInterface.this[UGUI requestingAgent, UGUI principal]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT rm.*, r.\"Powers\", r.\"Title\" FROM grouprolememberships AS rm INNER JOIN grouproles AS r ON rm.\"GroupID\" = r.\"GroupID\" AND rm.\"RoleID\" = r.\"RoleID\" WHERE rm.\"PrincipalID\" = @principalid", conn))
                    {
        ...
        List<GroupRolemember> IGroupRolemembersInterface.this[UGUI requestingAgent, UGI group, UUID roleID]
        {
            get
            {
                ...
                if (UUID.Zero == roleID)
                {
                    ...
                    {
                        conn.Open();
                        using (var cmd = new NpgsqlCommand("SELECT * FROM groupmemberships WHERE rm.\"GroupID\" = @groupid", conn))
                        {
                            cmd.Parameters.AddParameter("@groupid", group.ID);
                            using (NpgsqlDataReader reader = cmd.ExecuteReader())
                            {
                ...
                else
                {
                    ...
                    {
                        conn.Open();
                        using (var cmd = new NpgsqlCommand("SELECT rm.*, r.\"Powers\" FROM grouprolememberships AS rm INNER JOIN grouproles AS r ON rm.\"GroupID\" = r.\"GroupID\" AND rm.\"RoleID\" = r.\"RoleID\" WHERE rm.\"GroupID\" = @groupid AND rm.\"RoleID\" = @roleid", conn))
                        {
                            cmd.Parameters.AddParameter("@groupid", group.ID);
        ...
        bool IGroupRolemembersInterface.ContainsKey(UGUI requestingAgent, UGI group, UUID roleID, UGUI principal)
        {
            ...
            else
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT rm.\"GroupID\" FROM grouprolememberships AS rm INNER JOIN grouproles AS r ON rm.\"GroupID\" = r.\"GroupID\" AND rm.\"RoleID\" = r.\"RoleID\" WHERE rm.\"GroupID\" = @groupid AND rm.\"RoleID\" = @roleid and rm.\"PrincipalID\" = @principalid LIMIT 1", conn))
                    {
                        cmd.Parameters.AddParameter("@groupid", group.ID);
                        cmd.Parameters.AddParameter("@roleid", roleID);
                        cmd.Parameters.AddParameter("@principalid", principal.ID);
        ...
        void IGroupRolemembersInterface.Delete(UGUI requestingAgent, UGI group, UUID roleID, UGUI principal)
        {
            ...
            else
            {
                ...
                using (var conn = new NpgsqlConnection(m_ConnectionString))
                {
                    ...
                    {
                        using (var cmd = new NpgsqlCommand("UPDATE groupmemberships SET \"SelectedRoleID\"=@zeroid WHERE \"SelectedRoleID\" = @roleid AND \"GroupID\" = @groupid AND \"PrincipalID\" = @principalid", conn)
                        {
                        ...
                        {
                            using (var cmd = new NpgsqlCommand("DELETE FROM " + table + " WHERE \"GroupID\" = @groupid AND \"RoleID\" = @roleid AND \"PrincipalID\" = @principalid", conn)
                            {
                                Transaction = transaction
                            })
                            {
        ...
        bool IGroupRolemembersInterface.TryGetValue(UGUI requestingAgent, UGI group, UUID roleID, UGUI principal, out GroupRolemember grolemem)
        {
            ...
            else
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT rm.*, r.\"Powers\" FROM grouprolememberships AS rm INNER JOIN grouproles AS r ON rm.\"GroupID\" = r.\"GroupID\" AND rm.\"RoleID\" = r.\"RoleID\" WHERE rm.\"GroupID\" = @groupid AND rm.\"RoleID\" = @roleid and rm.\"PrincipalID\" = @principalid LIMIT 1", conn))
                    {
                        cmd.Parameters.AddParameter("@groupid", group.ID);
                        cmd.Parameters.AddParameter("@roleid", roleID);
                        cmd.Parameters.AddParameter("@principalid", principal.ID);
