    ...
    public sealed partial class PostgreSQLGroupsService : GroupsServiceInterface.IGroupRolemembersInterface
    {
        ...
        bool IGroupRolemembersInterface.ContainsKey(UUI requestingAgent, UGI group, UUID roleID, UUI principal)
        {
            ...
            else
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT rm.\"GroupID\" FROM grouprolememberships AS rm INNER JOIN grouproles AS r ON rm.\"GroupID\" = r.\"GroupID\" AND rm.\"RoleID\" = r.\"RoleID\" WHERE rm.\"GroupID\" = @groupid AND rm.\"RoleID\" = @roleid and rm.\"PrincipalID\" = @principalid", conn))
                    {
                        cmd.Parameters.AddParameter("@groupid", group.ID);
                        cmd.Parameters.AddParameter("@roleid", roleID);
                        cmd.Parameters.AddParameter("@principalid", principal.ID);
        ...
        bool IGroupRolemembersInterface.TryGetValue(UUI requestingAgent, UGI group, UUID roleID, UUI principal, out GroupRolemember grolemem)
        {
            ...
            else
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT rm.*, r.\"Powers\" FROM grouprolememberships AS rm INNER JOIN grouproles AS r ON rm.\"GroupID\" = r.\"GroupID\" AND rm.\"RoleID\" = r.\"RoleID\" WHERE rm.\"GroupID\" = @groupid AND rm.\"RoleID\" = @roleid and rm.\"PrincipalID\" = @principalid", conn))
                    {
                        cmd.Parameters.AddParameter("@groupid", group.ID);
                        cmd.Parameters.AddParameter("@roleid", roleID);
                        cmd.Parameters.AddParameter("@principalid", principal.ID);
