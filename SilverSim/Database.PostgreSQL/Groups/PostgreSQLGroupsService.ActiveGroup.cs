    ...
    public sealed partial class PostgreSQLGroupsService : GroupsServiceInterface.IGroupSelectInterface
    {
        UGI IGroupSelectInterface.this[UUI requestingAgent, UUI principalID]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT \"ActiveGroupID\" FROM activegroup WHERE \"PrincipalID\" = @principalid LIMIT 1", conn))
                    {
            ...
            set
            {
                if (Members.ContainsKey(requestingAgent, value, principalID))
                {
                    ...
                    {
                        conn.Open();
                        conn.ReplaceInto("activegroup", vals, new string[] { "PrincipalID", "ActiveGroupID" }, m_EnableOnConflict);
                    }
                }
            }
        }
        ...
        bool IGroupSelectInterface.TryGetValue(UUI requestingAgent, UGI group, UUI principal, out UUID id)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT \"SelectedRoleID\" FROM groupmemberships WHERE \"PrincipalID\" = @principalid AND \"GroupID\" = @groupid LIMIT 1", conn))
                {
                    cmd.Parameters.AddParameter("@groupid", group.ID);
                    cmd.Parameters.AddParameter("@principalid", principal.ID);
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
