...
{
    public sealed partial class PostgreSQLGroupsService : GroupsServiceInterface.IGroupInvitesInterface
    {
        List<GroupInvite> IGroupInvitesInterface.this[UGUI requestingAgent, UGUI principal]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT * from groupinvites WHERE \"PrincipalID\" = @principalid", conn))
                    {
        ...
        List<GroupInvite> IGroupInvitesInterface.this[UGUI requestingAgent, UGI group, UUID roleID, UGUI principal]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT * from groupinvites WHERE \"PrincipalID\" = @principalid AND \"GroupID\" = @groupid AND \"RoleID\" = @roleid", conn))
                    {
                        cmd.Parameters.AddParameter("@principalid", principal.ID);
        ...
        List<GroupInvite> IGroupInvitesInterface.GetByGroup(UGUI requestingAgent, UGI group)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT * from groupinvites WHERE \"GroupID\" = @groupid", conn))
                {
                    cmd.Parameters.AddParameter("@groupid", group.ID);
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
                    {
