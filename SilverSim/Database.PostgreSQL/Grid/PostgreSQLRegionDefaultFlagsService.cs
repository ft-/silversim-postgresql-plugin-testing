...
namespace SilverSim.Database.PostgreSQL.Grid
{
    ...
    public sealed class PostgreSQLRegionDefaultFlagsService : RegionDefaultFlagsServiceInterface, IPlugin, IDBServiceInterface
    {
        ...
        private static readonly ILog m_Log = LogManager.GetLogger("POSTGRESQL REGIONDEFAULTFLAGS SERVICE");
            ...
            using (var connection = new NpgsqlConnection(m_ConnectionString))
            {
                ...
                connection.InsideTransaction((transaction) =>
                {
                    ...
                    if (haveEntry)
                    {
                        ...
                        using (var cmd = new NpgsqlCommand("DELETE FROM regiondefaults WHERE \"flags\" = 0 AND \"uuid\" = @id", connection))
                        {
                            cmd.Parameters.AddParameter("@id", regionId);
                            cmd.ExecuteNonQuery();
                        }
        ...
        public override Dictionary<UUID, RegionFlags> GetAllRegionDefaultFlags()
        {
            ...
            {
                connection.Open();
                using (var cmd = new NpgsqlCommand("SELECT * FROM regiondefaults", connection))
                {
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
