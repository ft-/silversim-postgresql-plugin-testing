...
{
    public sealed partial class PostgreSQLGridService : IAssetReferenceInfoServiceInterface
    {
        public void EnumerateUsedAssets(Action<UUID> action)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT \"regionMapTexture\", \"parcelMapTexture\" FROM " + m_TableName, conn))
                {
