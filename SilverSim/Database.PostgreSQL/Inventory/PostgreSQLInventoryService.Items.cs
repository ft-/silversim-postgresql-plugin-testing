...
namespace SilverSim.Database.PostgreSQL.Inventory
{
    public partial class PostgreSQLInventoryService : IInventoryItemServiceInterface
    {
        ...
        bool IInventoryItemServiceInterface.TryGetValue(UUID key, out InventoryItem item)
        {
            ...
            {
                connection.Open();
                using (var cmd = new NpgsqlCommand("SELECT * FROM " + m_InventoryItemTable + " WHERE \"ID\" = @itemid", connection))
                {
                    cmd.Parameters.AddParameter("@itemid", key);
                    using (NpgsqlDataReader dbReader = cmd.ExecuteReader())
                    {
        ...
        bool IInventoryItemServiceInterface.TryGetValue(UUID principalID, UUID key, out InventoryItem item)
        {
            ...
            {
                connection.Open();
                using (var cmd = new NpgsqlCommand("SELECT * FROM " + m_InventoryItemTable + " WHERE \"OwnerID\" = @ownerid AND \"ID\" = @itemid", connection))
                {
                    cmd.Parameters.AddParameter("@ownerid", principalID);
                    cmd.Parameters.AddParameter("@itemid", key);
                    using (NpgsqlDataReader dbReader = cmd.ExecuteReader())
