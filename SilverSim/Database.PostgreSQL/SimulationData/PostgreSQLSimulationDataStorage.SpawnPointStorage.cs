...
{
    public sealed partial class PostgreSQLSimulationDataStorage : ISimulationDataSpawnPointStorageInterface
    {
        List<Vector3> ISimulationDataSpawnPointStorageInterface.this[UUID regionID]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT \"DistanceX\", \"DistanceY\", \"DistanceZ\" FROM spawnpoints WHERE \"RegionID\" = @regionid", conn))
                    {
            ...
            set
            {
                using (var conn = new NpgsqlConnection(m_ConnectionString))
                {
                    ...
                    {
                        using (var cmd = new NpgsqlCommand("DELETE FROM spawnpoints WHERE \"RegionID\" = @regionid", conn)
                        {
