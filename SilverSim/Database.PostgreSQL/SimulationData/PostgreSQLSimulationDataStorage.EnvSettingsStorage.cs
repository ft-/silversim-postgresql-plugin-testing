...
{
    public sealed partial class PostgreSQLSimulationDataStorage : ISimulationDataEnvSettingsStorageInterface
    {
        bool ISimulationDataEnvSettingsStorageInterface.TryGetValue(UUID regionID, out WindLightSettings settings)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT \"EnvironmentSettings\" FROM environmentsettings WHERE \"RegionID\" = @regionid", conn))
                {
            ...
            set
            {
                using (var conn = new NpgsqlConnection(m_ConnectionString))
                {
...
#endif
                        using (var cmd = new NpgsqlCommand("DELETE FROM environmentsettings WHERE \"RegionID\" = @regionid", conn))
                        {
                            cmd.Parameters.AddParameter("@regionid", regionID);
                            cmd.ExecuteNonQuery();
                        }
                    ...
                    else
                    {
                        ...
                        conn.ReplaceInto("environmentsettings", param, new string[] { "RegionID" }, m_EnableOnConflict);
                    }
                }
            }
        }
