...
{
    [Description("PostgreSQL Simulation Data Backend")]
    [PluginName("SimulationData")]
    public sealed partial class PostgreSQLSimulationDataStorage : SimulationDataStorageInterface, IDBServiceInterface, IPlugin, IQueueStatsAccess
    {
        private static readonly ILog m_Log = LogManager.GetLogger("POSTGRESQL SIMULATION STORAGE");
        ...
        public override void RemoveRegion(UUID regionID)
        {
            ...
            foreach (string table in Tables)
            {
                ...
                {
                    connection.Open();
                    using (var cmd = new NpgsqlCommand("DELETE FROM " + table + " WHERE \"RegionID\" = '" + regionIdStr + "'", connection))
                    {
                        cmd.ExecuteNonQuery();
                    }
                }
        ...
        IList<QueueStatAccessor> IQueueStatsAccess.QueueStats
        {
            get
            {
                ...
                foreach (PostgreSQLTerrainListener terListener in m_TerrainListenerThreads)
                {
                    statFuncs.Add(new QueueStatAccessor("TerrainStore." + terListener.RegionID.ToString(), terListener.GetStats));
                }

                foreach (PostgreSQLSceneListener sceneListener in m_SceneListenerThreads)
                {
