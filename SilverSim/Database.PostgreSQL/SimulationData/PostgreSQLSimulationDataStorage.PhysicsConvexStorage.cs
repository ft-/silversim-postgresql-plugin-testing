...
{
    public sealed partial class PostgreSQLSimulationDataStorage : ISimulationDataPhysicsConvexStorageInterface, IPhysicsHacdCleanCache
    {
        ...
        PhysicsConvexShape ISimulationDataPhysicsConvexStorageInterface.this[UUID meshid]
        {
            ...
            set
            {
                using (var conn = new NpgsqlConnection(m_ConnectionString))
                {
                    ...
                    conn.ReplaceInto("sculptmeshphysics", param, new string[] { "MeshID" }, m_EnableOnConflict);
                }
            }
        }
        ...
        bool ISimulationDataPhysicsConvexStorageInterface.TryGetValue(UUID meshid, out PhysicsConvexShape shape)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT ConvexData FROM meshphysics WHERE \"MeshID\"=@id", conn))
                {
        ...
        bool ISimulationDataPhysicsConvexStorageInterface.ContainsKey(UUID sculptmeshid)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT NULL FROM meshphysics WHERE \"MeshID\"=@id", conn))
                {
        ...
        bool ISimulationDataPhysicsConvexStorageInterface.ContainsKey(ObjectPart.PrimitiveShape primShape)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT NULL FROM primphysics WHERE \"ShapeKey\"=@id", conn))
                {
                    cmd.Parameters.AddParameter("@id", primShape.Serialization);
                    using (NpgsqlDataReader dbReader = cmd.ExecuteReader())
                    {
        ...
        bool ISimulationDataPhysicsConvexStorageInterface.Remove(UUID sculptmeshid)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("DELETE FROM meshphysics WHERE \"MeshID\"=@id", conn))
                {
                    cmd.Parameters.AddParameter("@id", sculptmeshid);
        ...
        ICollection<UUID> ISimulationDataPhysicsConvexStorageInterface.KnownMeshIds
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT \"MeshID\" FROM meshphysics", conn))
                    {
                        using (NpgsqlDataReader dbReader = cmd.ExecuteReader())
                        {
                            while (dbReader.Read())
