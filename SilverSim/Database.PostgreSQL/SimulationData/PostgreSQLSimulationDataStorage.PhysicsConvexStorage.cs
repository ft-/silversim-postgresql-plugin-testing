...
namespace SilverSim.Database.PostgreSQL.SimulationData
{
    public sealed partial class PostgreSQLSimulationDataStorage : ISimulationDataPhysicsConvexStorageInterface, IPhysicsHacdCleanCache
    {
        ...
        bool ISimulationDataPhysicsConvexStorageInterface.ContainsKey(UUID sculptmeshid, PrimitivePhysicsShapeType physicsShape)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT NULL FROM meshphysics WHERE \"MeshID\"=@id AND \"PhysicsShape\"=@stype", conn))
                {
        ...
        bool ISimulationDataPhysicsConvexStorageInterface.Remove(UUID sculptmeshid, PrimitivePhysicsShapeType physicsShape)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("DELETE FROM meshphysics WHERE \"MeshID\"=@id AND \"PhysicsShape\"=@stype", conn))
                {
                    cmd.Parameters.AddParameter("@id", sculptmeshid);
                    cmd.Parameters.AddParameter("@stype", physicsShape);
