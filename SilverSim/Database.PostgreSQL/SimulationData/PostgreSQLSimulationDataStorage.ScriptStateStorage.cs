...
{
    public sealed partial class PostgreSQLSimulationDataStorage : ISimulationDataScriptStateStorageInterface
    {
        ...
        bool ISimulationDataScriptStateStorageInterface.Remove(UUID regionID, UUID primID, UUID itemID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new NpgsqlCommand("DELETE FROM scriptstates WHERE \"RegionID\" = @regionid AND \"PrimID\" = @primid AND \"ItemID\" = @itemid", connection))
                {
                    cmd.Parameters.AddParameter("@regionid", regionID);
                    cmd.Parameters.AddParameter("@primid", primID);
                    cmd.Parameters.AddParameter("@itemid", itemID);
