...
{
    public sealed class PostgreSQLSimulationDataParcelAccessListStorage : ISimulationDataParcelAccessListStorageInterface
    {
        private readonly string m_ConnectionString;
        private readonly string m_TableName;
        private readonly bool m_EnableOnConflict;

        public PostgreSQLSimulationDataParcelAccessListStorage(string connectionString, string tableName, bool enableOnConflict)
        {
        ...
        public List<ParcelAccessEntry> this[UUID regionID, UUID parcelID]
        {
            get
            {
                ...
                using (var connection = new NpgsqlConnection(m_ConnectionString))
                {
                    ...
                    using (var cmd = new NpgsqlCommand("SELECT * FROM " + m_TableName + " WHERE \"RegionID\" = '" + regionID.ToString() + "' AND \"ParcelID\" = '" + parcelID.ToString() + "'", connection))
                    {
                        using (NpgsqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
        ...
        public void Store(ParcelAccessEntry entry)
        {
            using (var connection = new NpgsqlConnection(m_ConnectionString))
            {
                ...
                connection.ReplaceInto(m_TableName, data, new string[] { "RegionID", "ParcelID", "Accessor" }, m_EnableOnConflict);
            }
        }
        ...
        public void ExtendExpiry(UUID regionID, UUID parcelID, UGUI accessor, ulong extendseconds)
        {
            ...
            using (var connection = new NpgsqlConnection(m_ConnectionString))
            {
                ...
                connection.InsideTransaction((transaction) =>
                {
                    ...
                    using (var cmd = new NpgsqlCommand("INSERT INTO " + m_TableName + " (\"RegionID\", \"ParcelID\", \"Accessor\", \"ExpiresAt\") SELECT @RegionID, @ParcelID, @Accessor, @ExpiresAt "+
                    ...
                    using (var cmd = new NpgsqlCommand("UPDATE " + m_TableName + " SET \"ExpiresAt\" = \"ExpiresAt\" + @extendseconds WHERE \"RegionID\" = @RegionID AND \"ParcelID\" = @ParcelID AND \"Accessor\" = @Accessor AND \"ExpiresAt\" > 0", connection)
                    {
                        Transaction = transaction
                    })
                    {
        ...
        public bool RemoveAllFromRegion(UUID regionID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new NpgsqlCommand("DELETE FROM " + m_TableName + " WHERE \"RegionID\" = '" + regionID.ToString() + "'", connection))
                {
        ...
        public bool Remove(UUID regionID, UUID parcelID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new NpgsqlCommand("DELETE FROM " + m_TableName + " WHERE \"RegionID\" = '" + regionID.ToString() + "' AND \"ParcelID\" = '" + parcelID.ToString() + "'", connection))
                {
                    return cmd.ExecuteNonQuery() > 0;
                }
            }
