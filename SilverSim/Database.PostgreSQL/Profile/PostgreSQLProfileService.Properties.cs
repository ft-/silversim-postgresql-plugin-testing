...
{
    public sealed partial class PostgreSQLProfileService : ProfileServiceInterface.IPropertiesInterface
    {
        ProfileProperties IPropertiesInterface.this[UUI user]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT * FROM userprofile where \"useruuid\" = @uuid LIMIT 1", conn))
                    {
