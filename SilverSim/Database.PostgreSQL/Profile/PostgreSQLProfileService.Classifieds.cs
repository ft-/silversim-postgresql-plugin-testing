...
namespace SilverSim.Database.PostgreSQL.Profile
{
    public sealed partial class PostgreSQLProfileService : ProfileServiceInterface.IClassifiedsInterface
    {
        Dictionary<UUID, string> IClassifiedsInterface.GetClassifieds(UUI user)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT \"classifieduuid\", \"name\" FROM classifieds WHERE \"creatoruuid\" = @uuid", conn))
                {
        ...
        bool IClassifiedsInterface.TryGetValue(UUI user, UUID id, out ProfileClassified classified)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT * FROM classifieds WHERE \"classifieduuid\" = @uuid", conn))
                {
        ...
        bool IClassifiedsInterface.ContainsKey(UUI user, UUID id)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT \"classifieduuid\" FROM classifieds WHERE \"classifieduuid\" = @uuid", conn))
                {
                    cmd.Parameters.AddParameter("@uuid", id);
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
                    {
        ...
        void IClassifiedsInterface.Update(ProfileClassified c)
        {
            ...
            {
                conn.Open();
                conn.ReplaceInto("classifieds", replaceVals, new string[] { "classifieduuid" }, m_EnableOnConflict);
            }
        }
        ...
        void IClassifiedsInterface.Delete(UUID id)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("DELETE FROM classifieds WHERE \"classifieduuid\" = @classifieduuid", conn))
                {
