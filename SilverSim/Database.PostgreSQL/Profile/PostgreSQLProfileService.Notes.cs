...
namespace SilverSim.Database.PostgreSQL.Profile
{
    public sealed partial class PostgreSQLProfileService : ProfileServiceInterface.INotesInterface
    {
        ...
        bool INotesInterface.TryGetValue(UUI user, UUI target, out string notes)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT * FROM usernotes WHERE \"useruuid\" = @user AND \"targetuuid\" = @target", conn))
                {
                    cmd.Parameters.AddParameter("@user", user.ID);
                    cmd.Parameters.AddParameter("@target", target.ID);
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
        ...
        string INotesInterface.this[UUI user, UUI target]
        {
            ...
            set
            {
                ...
                {
                    conn.Open();
                    conn.ReplaceInto("usernotes", replaceVals, new string[] { "user", "target" }, m_EnableOnConflict);
                }
            }
        }
    }
