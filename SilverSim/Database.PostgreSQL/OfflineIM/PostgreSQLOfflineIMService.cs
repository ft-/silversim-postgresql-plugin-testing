...
namespace SilverSim.Database.PostgreSQL.OfflineIM
{
    [Description("PostgreSQL OfflineIM Backend")]
    [PluginName("OfflineIM")]
    public sealed class PostgreSQLOfflineIMService : OfflineIMServiceInterface, IPlugin, IDBServiceInterface, IUserAccountDeleteServiceInterface
    {
        private static readonly ILog m_Log = LogManager.GetLogger("POSTGRESQL OFFLINEIM SERVICE");
        ...
        public override void DeleteOfflineIM(ulong offlineImID)
        {
            ...
            {
                connection.Open();
                using (var command = new NpgsqlCommand("DELETE FROM offlineim WHERE \"ID\" = @id", connection))
                {
        ...
        public void Remove(UUID scopeID, UUID accountID)
        {
            ...
            {
                connection.Open();
                using (var command = new NpgsqlCommand("DELETE FROM offlineim WHERE \"ToAgentID\" = @id", connection))
                {
