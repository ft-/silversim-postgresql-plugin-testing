...
namespace SilverSim.Database.PostgreSQL.MuteList
{
    [Description("PostgreSQL MuteList Backend")]
    [PluginName("MuteList")]
    public sealed class PostgreSQLMuteListService : MuteListServiceInterface, IPlugin, IDBServiceInterface, IUserAccountDeleteServiceInterface
    {
        private static readonly ILog m_Log = LogManager.GetLogger("POSTGRESQL MUTE SERVICE");
        ...
        public void Remove(UUID scopeID, UUID accountID)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("DELETE FROM mutelists WHERE \"agentID\" = @agentid", conn))
                {
        ...
        public override bool Remove(UUID muteListOwnerID, UUID muteID, string muteName)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("DELETE FROM mutelists WHERE \"agentID\" = @agentid AND \"muteID\" = @muteid AND \"muteName\" = @mutename", conn))
                {
                    cmd.Parameters.AddParameter("@agentid", muteListOwnerID);
        ...
        public override void Store(UUID muteListOwnerID, MuteListEntry mute)
        {
            using (var conn = new NpgsqlConnection(m_ConnectionString))
            {
                ...
                conn.ReplaceInto("mutelists", vals, m_Keys, m_EnableOnConflict);
            }
        }
