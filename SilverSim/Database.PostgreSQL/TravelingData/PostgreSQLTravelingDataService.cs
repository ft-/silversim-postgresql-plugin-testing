...
namespace SilverSim.Database.PostgreSQL.TravelingData
{
    ...
    [Description("PostgreSQL TravelingData Backend")]
    [PluginName("TravelingData")]
    public sealed partial class PostgreSQLTravelingDataService : TravelingDataServiceInterface, IDBServiceInterface, IPlugin, IUserAccountDeleteServiceInterface
    {
        private static readonly ILog m_Log = LogManager.GetLogger("POSTGRESQL TRAVELINGDATA SERVICE");
        private readonly string m_ConnectionString;
        private readonly bool m_EnableOnConflict;

        public PostgreSQLTravelingDataService(IConfig ownSection)
        {
        ...
        public override TravelingDataInfo GetTravelingDatabyAgentUUIDAndNotHomeURI(UUID agentID, string homeURI)
        {
            ...
            {
                connection.Open();
                using (var cmd = new NpgsqlCommand("SELECT * FROM travelingdata WHERE \"UserID\" = @id AND (NOT \"GridExternalName\" = @homeuri) LIMIT 1", connection))
                {
        ...
        public override List<TravelingDataInfo> GetTravelingDatasByAgentUUID(UUID agentID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new NpgsqlCommand("SELECT * FROM travelingdata WHERE \"UserID\" = @id", connection))
                {
                    cmd.Parameters.AddParameter("@id", agentID);
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
                    {
        ...
        public override bool Remove(UUID sessionID, out TravelingDataInfo info)
        {
            ...
            using (var connection = new NpgsqlConnection(m_ConnectionString))
            {
                ...
                bool res = connection.InsideTransaction((transaction) =>
                {
                    ...
                    using (var cmd = new NpgsqlCommand("DELETE FROM travelingdata WHERE \"SessionID\" = @id", connection)
                    {
        ...
        public void Remove(UUID scopeID, UUID accountID)
        {
            ...
            {
                connection.Open();
                using (var cmd = new NpgsqlCommand("DELETE FROM travelingdata WHERE \"UserID\" = @id", connection))
                {
        ...
        public override bool RemoveByAgentUUID(UUID agentID, out TravelingDataInfo info)
        {
            ...
            using (var connection = new NpgsqlConnection(m_ConnectionString))
            {
                ...
                bool res = connection.InsideTransaction((transaction) =>
                {
                    using (var cmd = new NpgsqlCommand("SELECT * FROM travelingdata WHERE \"UserID\" = @id LIMIT 1", connection)
                    {
                        Transaction = transaction
                    })
                    {
