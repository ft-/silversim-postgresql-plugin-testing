...
{
    [Description("PostgreSQL UserAccount AvatarName backend")]
    [PluginName("UserAccountNames")]
    public class PostgreSQLUserAccountNameService : AvatarNameServiceInterface, IPlugin, IDBServiceInterface
    {
        ...
        public override List<UGUIWithName> Search(string[] names)
        {
            ...
            if (names.Length == 1)
            {
                ...
                {
                    connection.Open();
                    using (var cmd = new NpgsqlCommand("SELECT * FROM useraccounts WHERE \"FirstName\" LIKE @name AND \"LastName\" LIKE @name", connection))
                    {
            ...
            else if (names.Length == 2)
            {
                ...
                {
                    connection.Open();
                    using (var cmd = new NpgsqlCommand("SELECT * FROM useraccounts WHERE \"FirstName\" LIKE @name0 AND \"LastName\" LIKE @name1", connection))
                    {
        ...
        private UGUIWithName GetUGUIWithNameFromReader(NpgsqlDataReader reader) => new UGUIWithName
        {
        ...
        public override bool TryGetValue(UUID key, out UGUIWithName uui)
        {
            ...
            {
                connection.Open();
                using (var cmd = new NpgsqlCommand("SELECT \"ID\", \"FirstName\", \"LastName\" FROM useraccounts WHERE \"ID\" = @id LIMIT 1", connection))
                {
        ...
        public override bool TryGetValue(string firstName, string lastName, out UGUIWithName uui)
        {
            ...
            {
                connection.Open();
                using (var cmd = new NpgsqlCommand("SELECT \"ID\", \"FirstName\", \"LastName\" FROM useraccounts WHERE \"FirstName\" = @first AND \"LastName\" = @last LIMIT 1", connection))
                {
