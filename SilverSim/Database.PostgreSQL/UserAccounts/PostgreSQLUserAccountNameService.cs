...
namespace SilverSim.Database.PostgreSQL.UserAccounts
{
    ...
    public class PostgreSQLUserAccountNameService : AvatarNameServiceInterface, IPlugin, IDBServiceInterface
    {
        ...
        public override bool TryGetValue(UUID key, out UUI uui)
        {
            ...
            {
                connection.Open();
                using (var cmd = new NpgsqlCommand("SELECT \"ID\", \"FirstName\", \"LastName\" FROM useraccounts WHERE \"ID\" = @id", connection))
                {
        ...
        public override bool TryGetValue(string firstName, string lastName, out UUI uui)
        {
            ...
            {
                connection.Open();
                using (var cmd = new NpgsqlCommand("SELECT \"ID\", \"FirstName\", \"LastName\" FROM useraccounts WHERE \"FirstName\" = @first AND \"LastName\" = @last", connection))
                {
