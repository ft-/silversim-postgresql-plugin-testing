    ...
    public sealed partial class PostgreSQLExperienceService : ExperienceServiceInterface.IExperienceAdminInterface
    {
        List<UUID> IExperienceAdminInterface.this[UGUI agent]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT \"ExperienceID\", \"Admin\" FROM experienceadmins WHERE \"Admin\" LIKE @admin", conn))
                    {
        ...
        bool IExperienceAdminInterface.this[UUID experienceID, UGUI agent]
        {
            ...
            set
            {
                if (value)
                {
                    ...
                    {
                        conn.Open();
                        conn.ReplaceInto("experienceadmins", vals, new string[] { "ExperienceID", "Admin" }, m_EnableOnConflict);
                    }
                }
                else
                {
                    ...
                    {
                        conn.Open();
                        using (var cmd = new NpgsqlCommand("DELETE FROM experienceadmins WHERE \"ExperienceID\" = @experienceid AND \"Admin\" LIKE @admin", conn))
                        {
