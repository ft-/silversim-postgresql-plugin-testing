...
{
    public sealed partial class PostgreSQLEstateService : IEstateRegionMapServiceInterface
    {
        List<UUID> IEstateRegionMapServiceInterface.this[uint estateID]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT \"RegionID\" FROM estate_regionmap WHERE \"EstateID\" = @estateid", conn))
                    {
        ...
        bool IEstateRegionMapServiceInterface.Remove(UUID regionID)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("DELETE FROM estate_regionmap WHERE \"RegionID\" = @regionid", conn))
                {
                    cmd.Parameters.AddParameter("@regionid", regionID);
