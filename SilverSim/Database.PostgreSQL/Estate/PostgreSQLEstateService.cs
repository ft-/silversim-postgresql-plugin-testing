...
{
    [Description("PostgreSQL Estate Backend")]
    [PluginName("Estate")]
    public sealed partial class PostgreSQLEstateService : EstateServiceInterface, IDBServiceInterface, IPlugin
    {
        private readonly string m_ConnectionString;
        private static readonly ILog m_Log = LogManager.GetLogger("POSTGRESQL ESTATE SERVICE");
        private readonly bool m_EnableOnConflict;

        #region Constructor
        public PostgreSQLEstateService(IConfig ownSection)
        {
        ...
        public override EstateInfo this[uint estateID]
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    {
                        conn.Open();
                        using (var cmd = new NpgsqlCommand("DELETE FROM estates WHERE ID LIKE @id", conn))
                        {
                            cmd.Parameters.AddParameter("@id", estateID);
        ...
        public override List<EstateInfo> All
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT * FROM estates", conn))
                    {
                        using (NpgsqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
        ...
        public override List<uint> AllIDs
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT \"ID\" FROM estates", conn))
                    {
                        using (NpgsqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
