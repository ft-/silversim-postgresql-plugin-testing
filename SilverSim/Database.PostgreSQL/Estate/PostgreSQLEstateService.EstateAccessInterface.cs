...
{
    public sealed partial class PostgreSQLEstateService : IEstateAccessServiceInterface, IEstateAccessServiceListAccessInterface
    {
        List<UGUI> IEstateAccessServiceListAccessInterface.this[uint estateID]
        {
            get
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT \"UserID\" FROM estate_users WHERE \"EstateID\" = @estateid", conn))
                    {
