...
namespace SilverSim.Database.PostgreSQL.Estate
{
    public sealed partial class PostgreSQLEstateService : IEstateOwnerServiceInterface
    {
        bool IEstateOwnerServiceInterface.TryGetValue(uint estateID, out UUI uui)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT \"Owner\" FROM estates WHERE \"ID\" = @id LIMIT 1", conn))
                {
        ...
        UUI IEstateOwnerServiceInterface.this[uint estateID]
        {
            ...
            set
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("UPDATE estates SET \"Owner\" = @ownerid WHERE \"ID\" = @id", conn))
                    {
                        cmd.Parameters.AddParameter("@id", estateID);
